...
class PatternPart:
    def __init__(self, pattern):
        ...
        # TODO: Make sure blanks are treated appropriately
...
class BugFinder:
    def __init__(self):
        ...
        self.matches = 0
        # List of sets of pattern parts (one item per complete pattern found)
        self.matchParts = []
    ...
    def setPattern(self, fileName):
        try:
            ...
            lineNumber = 1
            for line in file:
                # Remove EOL and trailing blanks (leading spaces are assumed significant)
                line = line.rstrip()
...
#                 
                ...
                lineNumber += 1
            # TODO: Remove n leading spaces if all pattern lines have at least that many
    ...
    def findOccurrenceOfNextPatternPart(self, occurrenceList):
        ...
        for patternPart in self.pattern:
            ...
            for occurrence in patternPart.occurrences:
                # Skip occurrences that have already been used in another pattern
                if occurrence.used:
                    continue
        ...
        for patternFirstPart in self.pattern:
            ...
            for occurrence in patternFirstPart.occurrences:
                ...
                if len(occurrenceList) == len(self.pattern):
                    ...
                    self.matches += 1
                    # Invalidate the occurrences involved in the matching pattern
                    # so as to avoid reusing them in another match
                    for occurrence in occurrenceList:
                        occurrence.used = True
                    # Store the properties of the matching pattern parts for later use
